...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:6969/api/auth/signup', { email, password });
  ...
  const resendEmail = async () => {
    ...
    try {
      await axios.post('http://localhost:6969/api/auth/resend-verification', { email });
